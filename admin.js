...
// SUBSTITUA ESTES VALORES PELAS SUAS CREDENCIAIS DO FIREBASE CONSOLE
...
btnCadastrar.addEventListener('click', async () => {
  ...
  try {
    ...
    mostrarToast('Erro ao cadastrar. Verifique as regras do Firebase.', 'erro');
  } finally {
...
onSnapshot(qPresentes, (snapshot) => {
  ...
  mostrarToast('Erro ao carregar presentes. Verifique as regras do Firebase.', 'erro');
});

function renderizarTabela(presentes) {
  if (presentes.length === 0) {
    tabelaPresentes.innerHTML = `<tr><td colspan="5" class="vazio">Nenhum presente cadastrado. Adicione um acima!</td></tr>`;
    return;
  }
...
window.resetarReserva = async function(id) {
  ...
  try {
    ...
    mostrarToast('Erro ao remover reserva. Verifique as regras do Firebase.', 'erro');
  }
};
...
onSnapshot(qConfirmacoes, (snapshot) => {
  ...
  mostrarToast('Erro ao carregar confirmações. Verifique as regras do Firebase.', 'erro');
});
